...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {
		...
		ellipse(50, 460, 10, 10);
		line(45, 455, 55, 465);
		line(45, 465, 55, 455);
		
		fill(0, 0, 0);
		text("Last Day", 75, 460);
	}
		...
		for (Marker cm : countryMarkers) {
			...
			for (Marker qm : quakeMarkers) {
				...
				if (qm instanceof LandQuakeMarker) {
					LandQuakeMarker lQM = (LandQuakeMarker) qm;

					if (lQM.isOnLand && lQM.getCountry() == name)
						countryQuakeCounter++;
				}
			}
			landQuakeCounter += countryQuakeCounter;
			if (countryQuakeCounter > 0) {
				System.out.println(name + ": " + countryQuakeCounter);
			}
		}
		System.out.println("OCEAN QUAKES: " + (quakeMarkers.size() - landQuakeCounter));
